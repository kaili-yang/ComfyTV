...
class TestExecute:
    ...
    def test_selective_color(self, clip):
        cls = _classes()["SelectiveColorStage"]
        cls.execute(project_id="p1", sc_reds=0.5, video=clip)

    def test_split_colorfx_stages(self, clip):
        for name, kw in (("ChromaShiftStage", {'shift_rh': 6.0}),
                         ("PseudocolorStage", {'pseudo_preset': 'turbo'}),
                         ("PosterizeStage", {'elbg_colors': 4}),
                         ("GrayWorldStage", {})):
            _classes()[name].execute(project_id="p1", video=clip, **kw)
