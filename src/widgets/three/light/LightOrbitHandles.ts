...
import { orbitAnglesFor } from './lightTransform'
import { lightTarget, type LightInfoEntry, type Vec3 } from './types'
...
export type LightOrbitHandleType = 'yaw' | 'pitch' | 'distance'
...
function makeHandle(color: number, type: LightOrbitHandleType): THREE.Mesh {
...
export class LightOrbitHandles {
  ...
  update(light: LightInfoEntry | null): void {
    const visible = light?.type === 'directional'
    this.root.visible = visible
  ...
  setHovered(type: LightOrbitHandleType | null): void {
    const entries: [THREE.Mesh, THREE.Mesh, LightOrbitHandleType][] = [
  ...
  dragPlaneFor(type: LightOrbitHandleType, light: LightInfoEntry): THREE.Plane {
    const target = lightTarget(light)
    if (type === 'yaw') {
      return new THREE.Plane(new THREE.Vector3(0, 1, 0), -target.y)
    }
    ...
    return new THREE.Plane().setFromNormalAndCoplanarPoint(normal, vec(target))
  }
}
