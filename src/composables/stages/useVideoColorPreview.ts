...
import type { VideoColorParams } from '@/composables/stages/videoColorMath'
import { VideoColorRenderer } from '@/widgets/glsl/videoColorRenderer'

export interface VideoColorRendererLike {
  renderToCanvas(
...
export interface UseVideoColorPreviewOptions {
  videoEl: Ref<HTMLVideoElement | null>
  canvasEl: Ref<HTMLCanvasElement | null>
  nodeId?: string
  params: () => Partial<VideoColorParams>
  createRenderer?: () => VideoColorRendererLike
}

export function useVideoColorPreview(opts: UseVideoColorPreviewOptions) {
  const supported = ref(true)
  const unregister = opts.nodeId != null
    ? registerPreviewSource(opts.nodeId, () => opts.canvasEl.value)
    : null
  let renderer: VideoColorRendererLike | null = null
  let rafId = 0
  ...
  function renderOnce(): void {
    ...
    renderer ??= (opts.createRenderer ?? (() => new VideoColorRenderer()))()
