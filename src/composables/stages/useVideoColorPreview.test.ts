...
import {
  useVideoColorPreview,
  type VideoColorRendererLike,
} from './useVideoColorPreview'
...
function makeRenderer(ok = true) {
  const renderer: VideoColorRendererLike = {
...
describe('useVideoColorPreview', () => {
  ...
  function setup(opts: {
    ...
    renderer?: VideoColorRendererLike
  } = {}) {
    ...
    const params = reactive({ exposure: 0.5 })
    const renderer = opts.renderer ?? makeRenderer()
    const createRenderer = vi.fn(() => renderer)
    let api!: ReturnType<typeof useVideoColorPreview>
    const wrapper = mount(defineComponent({
      setup() {
        api = useVideoColorPreview({
          videoEl,
          canvasEl,
  ...
  it('renders once on attach when the video is ready and paused', () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenCalledWith(
      videoEl.value, { exposure: 0.5 }, canvasEl.value,
    )
  })
  ...
  it('re-renders when params change', async () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenLastCalledWith(
      expect.anything(), { exposure: 1.25 }, expect.anything(),
    )
  })
