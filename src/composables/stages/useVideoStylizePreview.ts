...
import type { VideoStylizeParams } from '@/composables/stages/videoStylizeMath'
import { VideoStylizeRenderer } from '@/widgets/glsl/videoStylizeRenderer'

export interface VideoStylizeRendererLike {
  renderToCanvas(
...
export interface UseVideoStylizePreviewOptions {
  videoEl: Ref<HTMLVideoElement | null>
  canvasEl: Ref<HTMLCanvasElement | null>
  nodeId?: string
  params: () => Partial<VideoStylizeParams>
  createRenderer?: () => VideoStylizeRendererLike
}

export function useVideoStylizePreview(opts: UseVideoStylizePreviewOptions) {
  const supported = ref(true)
  const unregister = opts.nodeId != null
    ? registerPreviewSource(opts.nodeId, () => opts.canvasEl.value)
    : null
  let renderer: VideoStylizeRendererLike | null = null
  let rafId = 0
  ...
  function renderOnce(): void {
    ...
    renderer ??= (opts.createRenderer ?? (() => new VideoStylizeRenderer()))()
