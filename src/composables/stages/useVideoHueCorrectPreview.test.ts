...
import {
  useVideoHueCorrectPreview,
  type VideoHueCorrectRendererLike,
} from './useVideoHueCorrectPreview'
...
function makeRenderer(ok = true) {
  const renderer: VideoHueCorrectRendererLike = {
...
describe('useVideoHueCorrectPreview', () => {
  ...
  function setup(opts: {
    ...
    renderer?: VideoHueCorrectRendererLike
    nodeId?: string
  } = {}) {
    ...
    const params = reactive({ curves: '', satThrsh: 0, luminanceMix: 0 })
    const renderer = opts.renderer ?? makeRenderer()
    const createRenderer = vi.fn(() => renderer)
    let api!: ReturnType<typeof useVideoHueCorrectPreview>
    const wrapper = mount(defineComponent({
      setup() {
        api = useVideoHueCorrectPreview({
          videoEl,
          canvasEl,
          nodeId: opts.nodeId,
          params: () => ({
            curves: params.curves,
            satThrsh: params.satThrsh,
            luminanceMix: params.luminanceMix,
          }),
          createRenderer,
        })
        return () => null
  ...
  it('renders once on attach when the video is ready and paused', () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenCalledWith(
      videoEl.value,
      { curves: '', satThrsh: 0, luminanceMix: 0 },
      canvasEl.value,
    )
  })
  ...
  it('re-renders on curve and slider edits', async () => {
    const { renderer, params, wrapper } = setup()
    params.curves = '{"hue":[[0,1],[0.5,1.3],[1,1]]}'
    ...
    expect(renderer.renderToCanvas).toHaveBeenLastCalledWith(
      expect.anything(),
      { curves: '{"hue":[[0,1],[0.5,1.3],[1,1]]}', satThrsh: 0.4, luminanceMix: 0 },
      expect.anything(),
    )
  })

  it('registers on the preview bus and unregisters on unmount', () => {
    const { canvasEl, wrapper } = setup({ nodeId: 'hc-42' })
    expect(getPreviewSource('hc-42')?.()).toBe(canvasEl.value)
    wrapper.unmount()
    expect(getPreviewSource('hc-42')).toBeNull()
  })
