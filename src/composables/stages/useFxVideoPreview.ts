...
export interface FxRendererLike<TParams> {
  renderToCanvas(
...
export interface UseFxVideoPreviewOptions<TParams> {
  videoEl: Ref<HTMLVideoElement | null>
  canvasEl: Ref<HTMLCanvasElement | null>
  nodeId?: string
  params: () => Partial<TParams>
  createRenderer: () => FxRendererLike<TParams>
}

export function useFxVideoPreview<TParams>(
  opts: UseFxVideoPreviewOptions<TParams>,
) {
  const supported = ref(true)
  const unregister = opts.nodeId != null
    ? registerPreviewSource(opts.nodeId, () => opts.canvasEl.value)
    : null
  let renderer: FxRendererLike<TParams> | null = null
  let rafId = 0
