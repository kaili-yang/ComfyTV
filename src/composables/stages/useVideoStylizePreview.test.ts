...
import {
  useVideoStylizePreview,
  type VideoStylizeRendererLike,
} from './useVideoStylizePreview'
...
function makeRenderer(ok = true) {
  const renderer: VideoStylizeRendererLike = {
...
describe('useVideoStylizePreview', () => {
  ...
  function setup(opts: {
    ...
    renderer?: VideoStylizeRendererLike
  } = {}) {
    ...
    const params = reactive({ effect: 'vignette' as const, strength: 0.5 })
    const renderer = opts.renderer ?? makeRenderer()
    const createRenderer = vi.fn(() => renderer)
    let api!: ReturnType<typeof useVideoStylizePreview>
    const wrapper = mount(defineComponent({
      setup() {
        api = useVideoStylizePreview({
          videoEl,
          canvasEl,
          params: () => ({ effect: params.effect, strength: params.strength }),
          createRenderer,
        })
        return () => null
      },
  ...
  it('renders once on attach when the video is ready and paused', () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenCalledWith(
      videoEl.value, { effect: 'vignette', strength: 0.5 }, canvasEl.value,
    )
  })
  ...
  it('re-renders when params change', async () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenLastCalledWith(
      expect.anything(), { effect: 'vignette', strength: 0.9 }, expect.anything(),
    )
  })
