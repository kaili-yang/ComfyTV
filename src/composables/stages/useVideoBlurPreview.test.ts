...
import {
  useVideoBlurPreview,
  type VideoBlurRendererLike,
} from './useVideoBlurPreview'
...
function makeRenderer(ok = true) {
  const renderer: VideoBlurRendererLike = {
...
describe('useVideoBlurPreview', () => {
  ...
  function setup(opts: {
    ...
    renderer?: VideoBlurRendererLike
  } = {}) {
    ...
    const params = reactive({ amount: 2 })
    const renderer = opts.renderer ?? makeRenderer()
    const createRenderer = vi.fn(() => renderer)
    let api!: ReturnType<typeof useVideoBlurPreview>
    const wrapper = mount(defineComponent({
      setup() {
        api = useVideoBlurPreview({
          videoEl,
          canvasEl,
  ...
  it('renders once on attach when the video is ready and paused', () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenCalledWith(
      videoEl.value, { amount: 2 }, canvasEl.value,
    )
  })
  ...
  it('re-renders when params change', async () => {
    ...
    expect(renderer.renderToCanvas).toHaveBeenLastCalledWith(
      expect.anything(), { amount: 7.5 }, expect.anything(),
    )
  })
