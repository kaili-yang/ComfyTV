...
class AudioDynamicsStage(io.ComfyNode):
    ...
    def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
        ...
        fx_spec = build_fx_spec("ComfyTV.AudioDynamicsStage", "Audio Dynamics",
                                "audio", [spec])
        if not src:
...
class AudioEQStage(io.ComfyNode):
    ...
    def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
        ...
        fx_spec = build_fx_spec("ComfyTV.AudioEQStage", "Audio EQ",
                                "audio", specs)
        if not src:
            return _fx_spec_only(fx_spec)
...
class AudioLoudnessStage(io.ComfyNode):
    ...
    def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
        ...
        payload = filter_audio(src, [spec], progress=cb, label=label)
        return _stage_emit_auto(cls, project_id=project_id, payload_str=payload,
                                parent_output_id=parent_output_id)
...
class AudioDenoiseStage(io.ComfyNode):
    ...
    def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
        ...
        fx_spec = build_fx_spec("ComfyTV.AudioDenoiseStage", "Audio Denoise",
                                "audio", [spec])
        if not src:
            return _fx_spec_only(fx_spec)
        payload = filter_audio(src, [spec], progress=_progress_cb(cls))
...
class AudioRepairStage(io.ComfyNode):
    ...
    def execute(cls, force_run_token=0, project_id="", parent_output_id=0,
        ...
        if method == 'hum':
            ...
            fx_spec = build_fx_spec("ComfyTV.AudioRepairStage", "Audio Repair",
                                    "audio", specs)
            if not src:
                return _fx_spec_only(fx_spec)
            payload = filter_audio(src, specs, progress=_progress_cb(cls))
